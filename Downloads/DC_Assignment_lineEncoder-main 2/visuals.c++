...
void renderMenu_digital()
{
    ...
    if (isMenuVisible)
    {
        for (int i = 0; i < numMenuItems; i++)
        {
            ...
            SDL_Surface *textSurface = TTF_RenderText_Solid(font, menuItems[i], color);
...
void renderMenu_analog()
{
    ...
    if (isMenuVisible)
    {
        for (int i = 0; i < numMenuItemsAnalog; i++)
        {
            ...
            // Render text for the item using SDL_ttf
            SDL_Color color =  {0, 0, 0, 255};
            SDL_Surface *textSurface = TTF_RenderText_Solid(font, menuItemsAnalog[i], color);
            SDL_Texture *textTexture = SDL_CreateTextureFromSurface(gRenderer, textSurface);
            SDL_Rect textRect = {800, 45 + i * MENU_HEIGHT, textSurface->w, textSurface->h};
            SDL_RenderCopy(gRenderer, textTexture, nullptr, &textRect);
            SDL_FreeSurface(textSurface);
...
void renderComboBox_digital()
{
    ...
    if (selectedMenuItem)
    {
        ...
        SDL_Surface *textSurface = TTF_RenderText_Solid(font, selectedMenuItem, color);
        SDL_Texture *textTexture = SDL_CreateTextureFromSurface(gRenderer, textSurface);
...
void renderForm_amplitude()
{
    ...
    SDL_Surface *ampLabelSurface = TTF_RenderText_Solid(font, "Amp", color);
